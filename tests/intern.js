...
define({
	...
	loader: {
		baseUrl: typeof process === 'undefined' ?
				// if we are using the full path to dmodel, we assume we are running
				// in a sibling path configuration
				location.search.indexOf('config=dmodel') > -1 ? '../..' : '..' :
			'./node_modules',
		...
		packages: [
			{ name: 'dojo', location: 'dojo' },
			{
				...
				location: typeof process === 'undefined' ?
					location.search.indexOf('config=dmodel') > -1 ? 'dmodel' : '..' :
					'..'
			},
